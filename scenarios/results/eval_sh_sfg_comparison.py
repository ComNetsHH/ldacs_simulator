...
def plot(json_filename, graph_filename_delays, graph_filename_distribution, graph_filename_comparison, time_slot_duration, sfg_csv_file):
	...
	with open(json_filename) as json_file:		
		...
		fig = plt.figure()		
		plt.plot(np.multiply(x, time_slot_duration), np.cumsum(y), label='analytical', linestyle='--', color='gray', zorder=1)
		plt.axvline(distribution_mean, linestyle='--', color='gray', linewidth=.5, label='analytical mean', zorder=0)
		plt.axvline(np.multiply(x, time_slot_duration)[nearest_99_index], linestyle='-.', color='gray', linewidth=.5, label='99\%', zorder=0)
		...
		plt.plot(x_shaped[0:max_i], y_shaped[0:max_i], label='analytical', linestyle='--', color='tab:blue', zorder=1)
		plt.axvline(distribution_mean, linestyle='--', color='tab:blue', linewidth=.75, zorder=0)
		plt.axvline(np.mean(all_vals), linestyle='--', color='tab:orange', linewidth=.75, zorder=0)		
		plt.plot(np.sort(all_vals), np.linspace(0, 1, len(all_vals)), color='tab:orange', label='empirical', zorder=1)
		plt.axvline(0, ymin=0, ymax=0, linestyle='--', color='k', linewidth=.75, label='means', zorder=0)		
