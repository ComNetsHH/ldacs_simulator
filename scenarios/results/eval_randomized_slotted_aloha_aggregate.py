...
def plot(json_filename, graph_filename_delays, graph_filename_reception, time_slot_duration, target_reception_rates, ylim1, ylim2):
	...
	with open(json_filename) as json_file:
		...
		line.remove()
		for j in range(n):
			line = plt.errorbar(num_users_vec, broadcast_delays_means[j]*time_slot_duration, broadcast_delays_err[j]*time_slot_duration, alpha=0.75 if target_reception_rates[j] != 37 else 1.0, fmt='o', color=colors[j])			
			plt.plot(num_users_vec, broadcast_delays_means[j]*time_slot_duration, linestyle='--', linewidth=1 if target_reception_rates[j] != 37 else 2, color=line[0].get_color(), alpha=0.75 if target_reception_rates[j] != 37 else 1.0, label=('$q=' + str((100-target_reception_rates[j])/100) + '$' if target_reception_rates[j] != 37 else '$q=1-\\frac{1}{e}$'))							
		for j in range(n):
			line = plt.errorbar(num_users_vec, avg_beacon_rx_mat_means[j]*time_slot_duration, avg_beacon_rx_mat_err[j]*time_slot_duration, alpha=0.75 if target_reception_rates[j] != 37 else 1.0, fmt='x', color=colors[j])
			plt.plot(num_users_vec, avg_beacon_rx_mat_means[j]*time_slot_duration, linestyle=':', linewidth=1 if target_reception_rates[j] != 37 else 2, color=line[0].get_color(), alpha=0.75 if target_reception_rates[j] != 37 else 1.0)		
		plt.ylabel('Delays [ms]')				
		plt.xlabel('Number of users $n$')		
		plt.legend(framealpha=0.0, prop={'size': 7}, loc='upper center', bbox_to_anchor=(.4, 1.3), ncol=3, columnspacing=0.5)				
		plt.yscale('log')		
		...
		fig = plt.figure()				
		for j in range(n):			
			line = plt.errorbar(num_users_vec, broadcast_reception_rate_means[j]*100, broadcast_reception_rate_err[j]*100, alpha=0.75 if target_reception_rates[j] != 37 else 1.0, fmt='o', label=('$q=' + str((100-target_reception_rates[j])/100) + '$' if target_reception_rates[j] != 37 else '$q=1-\\frac{1}{e}$'), color=colors[j])
			plt.plot(num_users_vec, broadcast_reception_rate_means[j]*100, linestyle='--' , linewidth=.5 if target_reception_rates[j] != 37 else 1.5, color=line[0].get_color(), alpha=0.75 if target_reception_rates[j] != 37 else 1.0)
