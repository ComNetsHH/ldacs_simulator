...
def plot(json_filename, graph_filename_delays, graph_filename_beacon_delays, graph_filename_reception, graph_filename_no_of_candidate_slots, graph_filename_selected_slot, graph_filename_collisions, graph_filename_duty_cycle, time_slot_duration, target_reception_rate, target_duty_cycle, ylim1, ylim2):
	...
	with open(json_filename) as json_file:
		...
		fig = plt.figure()		
		line = plt.errorbar(num_users_vec, avg_beacon_rx_mat_means*time_slot_duration, avg_beacon_rx_mat_err*time_slot_duration, alpha=0.75, fmt='o')
		...
		fig = plt.figure()				
		line = plt.errorbar(num_users_vec, broadcast_reception_rate_means*100, broadcast_reception_rate_err*100, alpha=0.75, fmt='o', label='simulation')
		plt.plot(num_users_vec, broadcast_reception_rate_means*100, linestyle='--', linewidth=.5, color=line[0].get_color(), alpha=0.75)
		...
		fig = plt.figure()				
		line = plt.errorbar(num_users_vec, broadcast_mean_num_candidate_slots_mat_means, broadcast_mean_num_candidate_slots_mat_err, alpha=0.75, fmt='o')
		plt.plot(num_users_vec, broadcast_mean_num_candidate_slots_mat_means, linestyle='--', linewidth=.5, color=line[0].get_color(), alpha=0.75)
		...
		fig = plt.figure()				
		line = plt.errorbar(num_users_vec, broadcast_mean_selected_slots_mat_means, broadcast_mean_selected_slots_mat_err, alpha=0.75, fmt='o')
		...
		fig = plt.figure()				
		line = plt.errorbar(num_users_vec, collision_rate_mat_means*100, collision_rate_mat_err*100, alpha=0.75, fmt='o', label='simulation')
		plt.plot(num_users_vec, collision_rate_mat_means*100, linestyle='--', linewidth=.5, color=line[0].get_color(), alpha=0.75)		
