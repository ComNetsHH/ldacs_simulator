...
def parse(dir, num_reps, num_users_per_swarm, json_filename):	
	...
	avg_collision_vals_1 = []
	avg_collision_vals_2 = []
	...
	for rep in range(num_reps):		
		try:
			...
			for i in range(num_users_per_swarm):
				...
				collision_vals = np.add([float(s) for s in collision_results_1['vecvalue'].values[0].split(' ')], [float(s) for s in collision_results_2['vecvalue'].values[0].split(' ')])
				tmp_avg_collisions[-1].append(collision_vals)
				if i == 0:
			...
			for t in range(len(tmp_avg_collisions[0][0])):
				avg_collision_vals_1.append(np.sum(tmp_avg_collisions[:,0,t]))
			bar_i += 1
			bar.update(bar_i)
	...
	json_data[json_label_num_collisions_vals_swarm_1] = np.array(avg_collision_vals_1).tolist()
...
def plot(json_filename, graph_filename_active_neighbors):
	"""
	...
	with open(json_filename) as json_file:		
		...
		avg_collision_vals = np.array(json_data[json_label_num_collisions_vals_swarm_1])
		...
		ax1.plot(times_vec, avg_neighbors_both, color='tab:blue')
		...
		ax2.set_ylabel('avg packet drops')		
...
if __name__ == "__main__":        	
	...
	if not args.no_plot:
		plot(json_filename, graph_filename_num_active_neighbors) 
